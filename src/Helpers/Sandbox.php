...
class Sandbox extends Strategy
{
    ...
    {
        return $this->invoice->authority ? "https://sandbox.zarinpal.com/pg/StartPay/" . $this->invoice->authority : null;
    }
    ...
    public function request(): string
    {
        ...
        if (!$this->invoice->isValidForRequest()) {
            throw new TransactionFailedException("Sandbox payment request failed due to incomplete/invalid transaction data. (exit code -422)", -422);
        }
        ...
        if ($bodyResponse != 100 || empty($result->Authority)) {
            throw new TransactionFailedException("Sandbox payment request failed. (exit code " . $bodyResponse . ")", $bodyResponse);
        }
    ...
    public function verify(): Receipt
    {
        if (!$this->invoice->isValidForValidation()) {
            throw new TransactionFailedException("Sandbox validation request failed due to incomplete/invalid transaction data. (exit code -422)", -422);
        }
        ...
        $bodyResponse = $result->Status;
        if ($bodyResponse != 100) {
            throw new TransactionFailedException("Sandbox validation request failed. (exit code " . $bodyResponse . ")", $bodyResponse);
        }
